...
const form = useForm({
  ...
  qty: '',
  unit_cost: '',
  moved_at: new Date().toISOString().slice(0,10), // YYYY-MM-DD
  note: '',
})
...
<template>
  <AuthenticatedLayout>
    <Head title="在庫異動 新規登録" />

    <div class="max-w-3xl mx-auto p-6 space-y-6">
      <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold">在庫異動 新規登録</h1>
      ...
      <form @submit.prevent="submit" class="space-y-5">
        <div>
          ...
          <select v-model="form.item_id" class="w-full border rounded px-3 py-2">
