...
<AuthenticatedLayout>
  ...
  <div class="max-w-7xl mx-auto p-6">
    ...
    <table class="min-w-full border border-gray-200">
      <thead class="bg-gray-100">
        <tr>
          ...
          <th class="px-4 py-2 border w-32">状態</th>
      ...
      <tbody>
        ...
        >
          <td class="px-4 py-2 border font-mono">{{ c.id }}</td>
          <td class="px-4 py-2 border">{{ c.name }}</td>
          <td class="px-4 py-2 border">
            <span v-if="c.is_active" class="text-green-600">有効</span>
            <span v-else class="text-gray-400">無効</span>
          </td>
          <td class="px-4 py-2 border">
           <Link :href="route('categories.show', c.id)" class="text-gray-700 underline">詳細</Link>
            <Link
              v-if="c.can?.update"
            ...
            <Link
              ...
              as="button"
              method="delete"
              :href="route('categories.destroy', c.id)"
    ...
    <!-- ページネーション -->
    <div class="mt-4">
      <div v-if="categories.links.length > 3" class="flex gap-2">
        <Link
